...
public:
    ...
    /**
     * Resize the board to the specified user input size.
    */
    void resizeBoard(int boardSize);

    /**
     * Randomly decides which cells of the vector are filled and which arent, using a
     * proportion of the total cells as the total filled cells.
    */
    void randomiseBoard(float percentage);

    /**
     * Attempts to use the given input to generate a board of certain size and proportion of cells filled.
     * Calls reiszeBoard() and randomiseBoard() to generate the board.
     * Returns TRUE if the generation succeeded.
     * Returns FALSE if the generation failed.
    */

    bool generateResult(std::vector<std::string> splitString);

    /**
     * Attempts to use the given input to initialise the player on the board.
     * Initialises the player by calling placeplayer() and initialiseplayer()
     * Returns TRUE if the initialisation has succeeded.
     * Returns FALSE if intialising the player failed.
    */
    bool initResult(Player* player, std::vector<std::string> splitString);
